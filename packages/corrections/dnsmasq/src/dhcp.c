...
static int make_fd(int port)
{
  ...
  struct timespec dly;
  dly.tv_sec = 0;
  dly.tv_nsec = 250000000;
  int tries = 1;
  int r = 1;
  while ((r = bind(fd, (struct sockaddr *)&saddr, sizeof(struct sockaddr_in))) != 0 && tries < 11) {
    my_syslog(LOG_INFO, _("failed to bind DHCP server socket, (retry:%d/10)"), tries);
    nanosleep(&dly, NULL);
    tries++;
  }
  if ( r != 0 )
