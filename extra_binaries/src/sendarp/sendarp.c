...
int main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "s:d:")) != -1) {
		switch (opt) {
		case 's':
			srcdev = strdup(optarg);
			break;
		case 'd':
			dstdev = strdup(optarg);
			break;
		}
	}
...
// This added by NT to avoid optimisation errors
static void set_int_into_4uchar(u_char *addr, u_int val) {
    u_char *addr2 = (u_char *)val;
    memcpy(addr, addr2, sizeof(u_int));
}
...
static void mkArpMsg(int opcode, u_long tInaddr, u_char *tHaddr,
	...
	set_int_into_4uchar(msg->sInaddr, sInaddr); //*((u_int *)msg->sInaddr) = sInaddr;		/* source IP address */
	bcopy(sHaddr, msg->sHaddr, 6);			/* source hardware address */
	set_int_into_4uchar(msg->tInaddr, tInaddr); //*((u_int *)msg->tInaddr) = tInaddr;		/* target IP address */
